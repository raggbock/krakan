...
export type HandleTakeoverStartDeps = {
  admin: SupabaseClient
  origin: string
  input: { token: string; email: string }
}

export async function handleTakeoverStart(
  deps: HandleTakeoverStartDeps,
): Promise<{ ok: true; magicLinkSent: boolean }> {
  const { admin, origin, input } = deps
...
definePublicEndpoint({
  ...
  handler: ({ admin, origin }, input) => handleTakeoverStart({ admin, origin, input }),
})
