...
export function useAdminMarketActivity(marketId: string | null) {
  return useQuery({
    ...
    queryFn: () => endpoints['admin.market.activity'].invoke({ marketId: marketId! }),
    enabled: !!marketId,
    staleTime: 10 * 1000,
  })
}
...
export function useAdminMarketsOverview() {
  return useQuery({
    ...
    queryFn: () => endpoints['admin.markets.overview'].invoke({}),
    staleTime: 30 * 1000,
  })
}
...
export function useAdminMarketEdit() {
  ...
  return useMutation({
    mutationFn: (input: AdminMarketEditInput) =>
      endpoints['admin.market.edit'].invoke(input),
    onSuccess: () => {
  ...
  return useMutation({
    mutationFn: async (input: { marketIds: string[]; patch: AdminMarketEditInput['patch'] }) => {
      for (const marketId of input.marketIds) {
        await endpoints['admin.market.edit'].invoke({ marketId, patch: input.patch })
      }
    },
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: queryKeys.admin.marketsOverview() })
