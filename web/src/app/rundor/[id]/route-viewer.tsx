...
type Props = {
  /** Pre-fetched route from server. Null in E2E_FAKE mode — falls back to useRoute(id). */
  route: RouteView | null
  id: string
}

export function RouteViewer({ route: initialRoute, id }: Props) {
  // E2E_FAKE fallback: when no server-side data was provided, resolve via hook.
  const { route: hookRoute, loading } = useRoute(initialRoute ? undefined : id)
  const route = initialRoute ?? hookRoute
