...
type Props = {
  initialMarkets: FleaMarketView[]
  initialCount: number
  initialPage: number
}

export default function ExploreClient({ initialMarkets, initialCount, initialPage }: Props) {
  ...
  const fallback = useMarkets({
    ...
    enabled: !userPos,
    // Seed the first page with server-rendered data so no extra round-trip
    // is needed on hydration when the user has not yet interacted.
    initialData: page === initialPage && !openNowOnly
      ? { items: initialMarkets, count: initialCount }
      : undefined,
  })
